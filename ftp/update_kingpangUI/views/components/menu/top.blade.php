...
@once
    ...
    @endpush
@endonce
<ul class="nav navbar-nav font-medium-2 text-bold-600">
    <li class="nav-item">
        <a data-toggle='' class="nav-link dropdown-toggle nav-link"
            href="{{ url(Category::findCode('blog')->link->pretty) }}">
            {{ Theme::title('explore') }}</a>
    </li>
    ...
    @if (isset($user))
        ...
        <li class="dropdown  nav-item">
            <a href="#" data-toggle="dropdown" class="nav-link">
                ...
                @else
                    <img loading="lazy" class="avatar avatar-online"
                        src="{{ asset('theme/images/portrait/small/avatar-s-1.png') }} " />
                @endif
            ...
            <ul class="dropdown-menu dropdown-menu-right">
                ...
                @if (isset($user->freelancer))
                    <li class="dropdown-item">
                        <a href="{{ route('freelancers.products.index', $user->freelancer) }}" class="nav-link">
                            {{ Theme::title('become a freelancer') }} </a>
                    </li>
                @else
                    <li class="dropdown-item">
                        <a href="{{ route('king') }}" class="nav-link"> {{ Theme::title('become a freelancer') }}
                        </a>
                    </li>
                @endif
